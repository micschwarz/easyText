...
if "bpy" in locals():
	import imp
	imp.reload(createtextletters)
	imp.reload(createtextword)
	imp.reload(font)
...
#Window: Create Text
class window_create (bpy.types.Panel):
...
#Window: Properties
class window_properties (bpy.types.Panel):
	bl_space_type = "VIEW_3D"
	bl_region_type = "TOOLS"
	bl_label = "Properties"
	bl_category = "easyText"
...
class window_options (bpy.types.Panel):
	bl_space_type = "VIEW_3D"
	bl_region_type = "TOOLS"
	bl_label = "Options"
	bl_category = "easyText"
...
class window_import_font (bpy.types.Panel):
	bl_space_type = "VIEW_3D"
	bl_region_type = "TOOLS"
...
class OBJECT_OT_buttonCreateLetters(bpy.types.Operator):
	bl_label = "Create"
	...
	def execute(self, context):
		s = bpy.context.scene
		createtextletters.create(s.content, s.spaceletters, font.search(s.font), s.extrude, s.offset, s.depth, s.resolution, s.mesh, s.edgesplit, s.uv)
		self.report({'INFO'}, "Text created")
		return{'FINISHED'}
		
class OBJECT_OT_buttonCreateWord(bpy.types.Operator):
	bl_label = "Create"
	...
	def execute(self, context):
		s = bpy.context.scene
		createtextword.create(s.content, s.spaceword, font.search(s.font), s.extrude, s.offset, s.depth, s.resolution, s.mesh, s.edgesplit, s.uv)
		self.report({'INFO'}, "Text created")
		return{'FINISHED'}

		
class OBJECT_OT_buttonLoadFont(bpy.types.Operator):
	bl_label = "Load Font"
	bl_idname = "load.font"
